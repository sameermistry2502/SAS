...
for Rental Enquiry call : +91 7359865986 or Submit your requirements below.
          </div>
          <br>
          <h3><span style="color: green">
                        ...
                        <?php 
                          ...
                          $projname=$_POST['comProj'];
                          $houseno=$_POST['comHno'];
                          $dateofpuss=$_POST['comDatePoss'];
                          $cname=$_POST['comName'];
                          $cemail=$_POST['comEmail'];
                          $cphone=$_POST['comPhone'];
                          $cstatus=$_POST['comStatus'];
                          $contype=$_POST['comType'];
                          $condetails=$_POST['comDetail'];
                          $cpriority=$_POST['comPriority'];
                          $combrief=$_POST['comDesc'];
                          $csolutiontime=$_POST['comTime'];
                          $cimg=$_POST['comFile'];


                          $con = mysqli_connect('localhost','root','','sas_db');

                           $quy = "INSERT INTO `complains`(`cprojectname`, `chouseno`, `cpossdate`, `cname`, `cemail`, `ccontactno`, `cstatus`, `ctype`, `cdetail`, `cpriority`, `cinbrief`, `csolutiontime`, `cimg`) VALUES ('$projname','$houseno','$dateofpuss','$cname','$cemail','$cphone','$cstatus','$contype','$condetails','$cpriority','$combrief','$csolutiontime','$cimg')";
                           
                          $run = mysqli_query($con,$quy);

                          if ($run == TRUE) 
                          {
                            echo "*Comaplin is Submitted. -><a href='complain.php'>Comaplain-Form</a>.....*";
                          }
                          else
                          {
